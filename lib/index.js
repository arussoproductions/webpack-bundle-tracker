...
class BundleTrackerPlugin {
  ...
  _handleDone(compiler, stats) {
    ...
    each(stats.compilation.assets, (file, assetName) => {
      ...
      if (this.options.publicPath || this.options.assetNameAsPublicPath) {
        if (this.options.assetNameAsPublicPath) {
          fileInfo.publicPath = this.options.assetNameAsPublicPath(assetName)
        } else {
